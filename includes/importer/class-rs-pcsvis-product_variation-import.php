...
class wc_PCSVIS_Product_Variation_Import extends wc_PCSVIS_Product_Import {
	...
	public function process_product( $post ) {
		...
		if ( empty( $post['post_parent'] ) ) {
			$this->add_import_result( 'skipped', __( 'No product variation parent set', 'rspider-csv-export-import-woo' ), $processing_product_id, 'Not set', $post['sku'] );
			wc_Product_CSV_Import_Suite::log( __('> Skipping - no post parent set.', 'rspider-csv-export-import-woo') );
			return;
		}

		if ( ! empty( $processing_product_id ) && isset( $this->processed_posts[ $processing_product_id ] ) ) {
			$this->add_import_result( 'skipped', __( 'Product variation already processed', 'rspider-csv-export-import-woo' ), $processing_product_id, get_the_title( $post['post_parent'] ), $post['sku'] );
			wc_Product_CSV_Import_Suite::log( __('> Post ID already processed. Skipping.', 'rspider-csv-export-import-woo') );
			return;
		}

		if ( $post['post_status'] == 'auto-draft' ){
			$this->add_import_result( 'skipped', __( 'Skipping auto-draft', 'rspider-csv-export-import-woo' ), $processing_product_id, get_the_title( $post['post_parent'] ), $post['sku'] );
			wc_Product_CSV_Import_Suite::log( __('> Skipping auto-draft.', 'rspider-csv-export-import-woo') );
			return;
		}
		...
		if ( ! $post_parent_exists ) {
			$this->add_import_result( 'failed', __( 'Variation parent does not exist', 'rspider-csv-export-import-woo' ), $processing_product_id, 'Does not exist', $post['sku'] );
			wc_Product_CSV_Import_Suite::log( sprintf( __('> Variation parent does not exist! (#%d)', 'rspider-csv-export-import-woo'), $post_parent ) );
			return;
		}

		// Check post type to avoid conflicts with IDs
		if ( $merging && get_post_type( $processing_product_id ) !== 'product_variation' ) {
			$this->add_import_result( 'skipped', __( 'Post is not a product variation', 'rspider-csv-export-import-woo' ), $processing_product_id, 'Not a variation', $post['sku'] );
			wc_Product_CSV_Import_Suite::log( sprintf( __('> &#8220;%s&#8221; is not a product variation.', 'rspider-csv-export-import-woo'), $processing_product_id ), true );
			unset( $post );
			return;
		}
		...
		if ( $merging ) {
			...
			wc_Product_CSV_Import_Suite::log( sprintf( __('> Merging post ID %s.', 'rspider-csv-export-import-woo'), $post_id ) );
			...
			if ( sizeof( $postdata ) ) {
				if ( wp_update_post( $postdata ) ) {
					...
					wc_Product_CSV_Import_Suite::log( __( '> Failed to merge post data: ', 'rspider-csv-export-import-woo' ) . print_r( $postdata, true ) );
				}
			}
			...
			if ( $this->variation_exists( $post_parent, $processing_product_id, $processing_product_sku ) ) {
				$this->add_import_result( 'skipped', __( 'Variation already exists', 'rspider-csv-export-import-woo' ), $processing_product_id, get_the_title( $post['post_parent'] ), $processing_product_sku );
				wc_Product_CSV_Import_Suite::log( sprintf( __( '> &#8220;%s&#8221; already exists.', 'rspider-csv-export-import-woo' ), esc_html( $post['post_title'] ) ), true );
				unset( $post );
				return;
			}

			// Insert product
			wc_Product_CSV_Import_Suite::log( __('> Inserting variation.', 'rspider-csv-export-import-woo') );
			...
			if ( is_wp_error( $post_id ) ) {
				...
				$this->add_import_result( 'failed', __( 'Failed to import product variation', 'rspider-csv-export-import-woo' ), $processing_product_id, get_the_title( $post['post_parent'] ), $post['sku'] );

				wc_Product_CSV_Import_Suite::log( sprintf( __( 'Failed to import product &#8220;%s&#8221;', 'rspider-csv-export-import-woo' ), esc_html($post['post_title']) ) );
				return;
		...
		if ( ! empty( $post['images'] ) ) {
			...
			if ( $merging ) {
				...
				wc_Product_CSV_Import_Suite::log( __( '> > Old images processed', 'rspider-csv-export-import-woo' ) );
		...
		if ( $merging ) {
			...
			wc_Product_CSV_Import_Suite::log( sprintf( __('> Finished merging variation ID %s.', 'rspider-csv-export-import-woo'), $post_id ) );
		} else {
			$this->add_import_result( 'imported', 'Import successful', $post_id, get_the_title( $post_parent ), $post['sku'] );
			wc_Product_CSV_Import_Suite::log( sprintf( __('> Finished importing variation ID %s.', 'rspider-csv-export-import-woo'), $post_id ) );
		}
	...
	function import_start( $file, $mapping, $start_pos, $end_pos ) {
		wc_Product_CSV_Import_Suite::log( __( 'Parsing product variations CSV.', 'rspider-csv-export-import-woo' ) );
		...
		wc_Product_CSV_Import_Suite::log( __( 'Finished parsing product variations CSV.', 'rspider-csv-export-import-woo' ) );
	...
	function header() {
		...
		echo '<h2>' . ( empty( $_GET['merge'] ) ? __( 'Import Product Variations', 'rspider-csv-export-import-woo' ) : __( 'Merge Product Variations', 'rspider-csv-export-import-woo' ) ) . '</h2>';
	}
