...
class wc_PCSVIS_Product_Import extends WP_Importer {
		...
		switch ( $step ) {
			...
			case 1 :
				...
				if ( $this->handle_upload() )
					...
					_e( 'Error with handle_upload!', 'rspider-csv-export-import-woo' );
			break;
			case 2 :
				$this->header();
				...
				if ( $file ) {
					...
					<table id="import-progress" class="widefat_importer widefat">
						<thead>
							<tr>
								<th class="status">&nbsp;</th>
								<th class="row"><?php _e( 'Row', 'rspider-csv-export-import-woo' ); ?></th>
								<th><?php _e( 'SKU', 'rspider-csv-export-import-woo' ); ?></th>
								<th><?php _e( 'Product', 'rspider-csv-export-import-woo' ); ?></th>
								<th class="reason"><?php _e( 'Status Msg', 'rspider-csv-export-import-woo' ); ?></th>
							</tr>
						</thead>
						<tfoot>
								...
								return $.ajax({
									...
									success:    function( response ) {
										...
										if ( response ) {
											...
											$('#import-progress tbody').append( '<tr class="error"><td class="status" colspan="5">' + '<?php _e( 'AJAX Error', 'rspider-csv-export-import-woo' ); ?>' + '</td></tr>' );
										}
								...
								$.ajax({
									...
									success: function( response ) {
										if ( response !== Object( response ) || ( typeof response.success === "undefined" && typeof response.error === "undefined" ) ) {
											...
											response.error = "<?php printf( esc_js( __( 'The resize request was abnormally terminated (ID %s). This is likely due to the image exceeding available memory or some other type of fatal error.', 'rspider-csv-export-import-woo' ) ), '" + id + "' ); ?>";
										}

										regen_count ++;

										$('#import-progress tbody .regenerating .progress').css( 'width', ( ( regen_count / attachments.length ) * 100 ) + '%' ).html( regen_count + ' / ' + attachments.length + ' <?php echo esc_js( __( 'thumbnails regenerated', 'rspider-csv-export-import-woo' ) ); ?>' );
					...
					echo '<p class="error">' . __( 'Error finding uploaded file!', 'rspider-csv-export-import-woo' ) . '</p>';
				}
			break;
			case 3 :
			...
			case 4 :
				...
				_e( 'Cleaning up...', 'rspider-csv-export-import-woo' ) . ' ';

				wp_defer_term_counting( true );
				wp_defer_comment_counting( true );

				_e( 'Clearing transients...', 'rspider-csv-export-import-woo' ) . ' ';
				...
				_e( 'Backfilling parents...', 'rspider-csv-export-import-woo' ) . ' ';

				$this->backfill_parents();

				if ( ! empty( $this->upsell_skus ) ) {
					_e( 'Linking upsells...', 'rspider-csv-export-import-woo' ) . ' ';
				...
				if ( ! empty( $this->crosssell_skus ) ) {
					_e( 'Linking crosssells...', 'rspider-csv-export-import-woo' ) . ' ';
				...
				if ( 'woocommerce_variation_csv' === $this->import_page && ! empty( $this->processed_posts ) ) {
					...
					_e( 'Syncing variations...', 'rspider-csv-export-import-woo' ) . ' ';
				...
				// SUCCESS
				_e( 'Finished. Import complete.', 'rspider-csv-export-import-woo' );
	...
	public function import() {
		...
		wc_Product_CSV_Import_Suite::log( __( 'Processing products.', 'rspider-csv-export-import-woo' ) );
		...
		wc_Product_CSV_Import_Suite::log( __( 'Finished processing products.', 'rspider-csv-export-import-woo' ) );
	}
	...
	public function import_start( $file, $mapping, $start_pos, $end_pos ) {
		...
		wc_Product_CSV_Import_Suite::log( __( 'Parsing products CSV.', 'rspider-csv-export-import-woo' ) );
		...
		wc_Product_CSV_Import_Suite::log( __( 'Finished parsing products CSV.', 'rspider-csv-export-import-woo' ) );
	...
	public function handle_upload() {
		...
		if ( empty( $_POST['file_url'] ) ) {
			...
			if ( isset( $file['error'] ) ) {
				echo '<p><strong>' . __( 'Sorry, there has been an error.', 'rspider-csv-export-import-woo' ) . '</strong><br />';
			...
			if ( file_exists( ABSPATH . $_POST['file_url'] ) ) {
				...
				echo '<p><strong>' . __( 'Sorry, there has been an error.', 'rspider-csv-export-import-woo' ) . '</strong></p>';
				return false;
	...
	public function process_product( $post ) {
		...
		if ( ! empty( $processing_product_id ) && isset( $this->processed_posts[ $processing_product_id ] ) ) {
			$this->add_import_result( 'skipped', __( 'Product already processed', 'rspider-csv-export-import-woo' ), $processing_product_id, $processing_product_title, $processing_product_sku );
			wc_Product_CSV_Import_Suite::log( __('> Post ID already processed. Skipping.', 'rspider-csv-export-import-woo'), true );
			unset( $post );
			return;
		}

		if ( ! empty ( $post['post_status'] ) && $post['post_status'] == 'auto-draft' ) {
			$this->add_import_result( 'skipped', __( 'Skipping auto-draft', 'rspider-csv-export-import-woo' ), $processing_product_id, $processing_product_title, $processing_product_sku );
			wc_Product_CSV_Import_Suite::log( __('> Skipping auto-draft.', 'rspider-csv-export-import-woo'), true );
			unset( $post );
			return;
		}
		...
		if ( ! $merging ) {
			if ( $this->product_exists( $processing_product_title, $processing_product_sku, $post['post_name'] ) ) {
				$this->add_import_result( 'skipped', __( 'Product already exists', 'rspider-csv-export-import-woo' ), $processing_product_id, $processing_product_title, $processing_product_sku );
				wc_Product_CSV_Import_Suite::log( sprintf( __('> &#8220;%s&#8221; already exists.', 'rspider-csv-export-import-woo'), esc_html($processing_product_title) ), true );
				unset( $post );
				return;
			}
			if ( $processing_product_id && is_string( get_post_status( $processing_product_id ) ) ) {
				$this->add_import_result( 'skipped', __( 'Importing post ID conflicts with an existing post ID', 'rspider-csv-export-import-woo' ), $processing_product_id, get_the_title( $processing_product_id ), '' );
				wc_Product_CSV_Import_Suite::log( sprintf( __('> &#8220;%s&#8221; ID already exists.', 'rspider-csv-export-import-woo'), esc_html( $processing_product_id ) ), true );
				unset( $post );
				return;
			}
		}
		...
		if ( $merging && $processing_product_id && get_post_type( $processing_product_id ) !== 'product' ) {
			$this->add_import_result( 'skipped', __( 'Post is not a product', 'rspider-csv-export-import-woo' ), $processing_product_id, $processing_product_title, $processing_product_sku );
			wc_Product_CSV_Import_Suite::log( sprintf( __('> &#8220;%s&#8221; is not a product.', 'rspider-csv-export-import-woo'), esc_html($processing_product_id) ), true );
			unset( $post );
			return;
		}
		...
		if ( $merging ) {
			...
			wc_Product_CSV_Import_Suite::log( sprintf( __('> Merging post ID %s.', 'rspider-csv-export-import-woo'), $post_id ), true );
			...
			if ( sizeof( $postdata ) > 1 ) {
				...
				if ( is_wp_error( $result ) ) {
					...
					wc_Product_CSV_Import_Suite::log( sprintf( __('> Failed to update product %s', 'rspider-csv-export-import-woo'), $post_id ), true );
					unset( $post );
					return;
				} else {
					wc_Product_CSV_Import_Suite::log( __( '> Merged post data: ', 'rspider-csv-export-import-woo' ) . print_r( $postdata, true ) );
				}
			}
			...
			// Insert product
			wc_Product_CSV_Import_Suite::log( sprintf( __('> Inserting %s', 'rspider-csv-export-import-woo'), esc_html( $processing_product_title ) ), true );
			...
			if ( is_wp_error( $post_id ) ) {
				$this->add_import_result( 'failed', __( 'Failed to import product', 'rspider-csv-export-import-woo' ), $processing_product_id, $processing_product_title, $processing_product_sku );
				wc_Product_CSV_Import_Suite::log( sprintf( __( 'Failed to import product &#8220;%s&#8221;', 'rspider-csv-export-import-woo' ), esc_html($processing_product_title) ) );
				unset( $post );
				return;
			} else {
				wc_Product_CSV_Import_Suite::log( sprintf( __('> Inserted - post ID is %s.', 'rspider-csv-export-import-woo'), $post_id ) );
			}
		}
		...
		if ( ! empty( $post['images'] ) && is_array( $post['images'] ) ) {
					...
					if ( in_array( $attachment_url, $post['images'] ) || in_array( $attachment_basename, $image_basenames ) ) {
						foreach( $post['images'] as $key => $image ) {
							if ( $image == $attachment_url || basename( $image ) == $attachment_basename ) {
								...
								wc_Product_CSV_Import_Suite::log( sprintf( __( '> > Image exists - skipping %s', 'rspider-csv-export-import-woo' ), basename( $image ) ) );
			...
			if ( $post['images'] ) foreach ( $post['images'] as $image_key => $image ) {
				...
				wc_Product_CSV_Import_Suite::log( sprintf( __( '> > Importing image "%s"', 'rspider-csv-export-import-woo' ), $image ) );
				...
				if ( ! is_wp_error( $attachment_id ) && $attachment_id ) {
					wc_Product_CSV_Import_Suite::log( sprintf( __( '> > Imported image "%s"', 'rspider-csv-export-import-woo' ), $image ) );
					...
					wc_Product_CSV_Import_Suite::log( sprintf( __( '> > Error importing image "%s"', 'rspider-csv-export-import-woo' ), $image ) );
			...
			wc_Product_CSV_Import_Suite::log( __( '> > Images set', 'rspider-csv-export-import-woo' ) );
		...
		if ( $merging ) {
			...
			wc_Product_CSV_Import_Suite::log( sprintf( __('> Finished merging post ID %s.', 'rspider-csv-export-import-woo'), $post_id ) );
		} else {
			$this->add_import_result( 'imported', 'Import successful', $post_id, $processing_product_title, $processing_product_sku );
			wc_Product_CSV_Import_Suite::log( sprintf( __('> Finished importing post ID %s.', 'rspider-csv-export-import-woo'), $post_id ) );
		}
	...
	public function process_attachment( $post, $url, $post_id ) {
		...
		if ( ! is_wp_error( $attachment_id ) && $attachment_id > 0 ) {
			wc_Product_CSV_Import_Suite::log( sprintf( __( '> > Inserted image attachment "%s"', 'rspider-csv-export-import-woo' ), $url ) );
	...
	public function fetch_remote_file( $url, $post ) {
		...
		if ( 0 == $filesize ) {
			@unlink( $upload['file'] );
			unset( $upload );
			return new WP_Error( 'import_file_error', __('Zero size file downloaded', 'rspider-csv-export-import-woo') );
		}
	...
	public function header() {
		...
		echo '<h2>' . ( empty( $_GET['merge'] ) ? __( 'Import Products', 'rspider-csv-export-import-woo' ) : __( 'Merge Products', 'rspider-csv-export-import-woo' ) ) . '</h2>';
	}
