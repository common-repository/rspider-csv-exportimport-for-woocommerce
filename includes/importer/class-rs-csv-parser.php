...
class wc_CSV_Parser {
	...
	public function parse_product( $item, $merge_empty_cells = 0 ) {
		...
		if ( $merging ) {
			...
			wc_Product_CSV_Import_Suite::log( sprintf( __( '> Row %s - preparing for merge.', 'rspider-csv-export-import-woo' ), $this->row ) );

			// Required fields
			if ( ! $post_id && empty( $item['sku'] ) ) {
				wc_Product_CSV_Import_Suite::log( __( '> > Cannot merge without id or sku. Importing instead.', 'rspider-csv-export-import-woo') );
				$merging = false;
			} else {
				...
				if ( ! $post_id ) {
					...
					if (  $is_variation && ! $found_product_id && empty( $item[ 'post_title' ] ) ) {
						...
						wc_Product_CSV_Import_Suite::log( sprintf(__( '> > Skipped. Cannot find product with sku %s.', 'rspider-csv-export-import-woo'), $item['sku']) );
						return new WP_Error( 'parse-error', __( 'Skipped. Cannot find product with sku.', 'rspider-csv-export-import-woo' ) );
					} elseif ( ! $found_product_id ) {
						wc_Product_CSV_Import_Suite::log( sprintf(__( '> > Skipped. Cannot find product with sku %s. Importing instead.', 'rspider-csv-export-import-woo'), $item['sku']) );
						$merging = false;
					} else {
						$post_id = $found_product_id;
						wc_Product_CSV_Import_Suite::log( sprintf(__( '> > Found product with ID %s.', 'rspider-csv-export-import-woo'), $post_id) );
					}
				}
			}
		}
		...
		if ( ! $merging ) {
			...
			wc_Product_CSV_Import_Suite::log( sprintf( __('> Row %s - preparing for import.', 'rspider-csv-export-import-woo'), $this->row ) );

			// Required fields
			if ( $this->post_type == 'product' && empty( $item['post_title'] ) ) {
				wc_Product_CSV_Import_Suite::log( __( '> > Skipped. No post_title set for new product.', 'rspider-csv-export-import-woo') );
				return new WP_Error( 'parse-error', __( 'No post_title set for new product.', 'rspider-csv-export-import-woo' ) );
			}
		...
		foreach ( $item as $key => $value ) {
			...
			elseif ( strstr( $key, 'tax:' ) ) {
				...
				if ( ! taxonomy_exists( $taxonomy ) ) {
					wc_Product_CSV_Import_Suite::log( sprintf( __('> > Skipping taxonomy "%s" - it does not exist.', 'rspider-csv-export-import-woo'), $taxonomy ) );
					continue;
				}
				...
				if ( 'product_type' === $taxonomy ) {
					...
					if ( ! array_key_exists( $term, $this->allowed_product_types ) ) {
						wc_Product_CSV_Import_Suite::log( sprintf( __('> > > Product type "%s" not allowed - using simple.', 'rspider-csv-export-import-woo'), $term ) );
					...
					if ( strstr( $raw_term, '>' ) ) {
						...
						foreach ( $raw_term as $term ) {
							...
							if ( isset( $this->inserted_terms[ $taxonomy ][ $parent ][ $term ] ) ) {
								...
								wc_Product_CSV_Import_Suite::log( sprintf( __( '> > (' . __LINE__ . ') Term %s (%s) exists? %s', 'rspider-csv-export-import-woo' ), sanitize_text_field( $term ), esc_html( $taxonomy ), $term_may_exist ? print_r( $term_may_exist, true ) : '-' ) );
								...
								if ( ! $term_id ) {
									...
									if ( ! is_wp_error( $t ) ) {
										...
										wc_Product_CSV_Import_Suite::log( sprintf( __( '> > (' . __LINE__ . ') Failed to import term %s, parent %s - %s', 'rspider-csv-export-import-woo' ), sanitize_text_field( $term ), sanitize_text_field( $parent ), sanitize_text_field( $taxonomy ) ) );
										break;
									}
								}
						...
						if ( isset( $this->inserted_terms[ $taxonomy ][0][$raw_term] ) ) {
							...
							if ( ! $term_id ) {
								...
								if ( ! is_wp_error( $t ) ) {
									...
									wc_Product_CSV_Import_Suite::log( sprintf( __( '> > Failed to import term %s %s', 'rspider-csv-export-import-woo' ), esc_html($raw_term), esc_html($taxonomy) ) );
									break;
								}
							}
			...
			elseif ( strstr( $key, 'attribute:' ) ) {
				...
				if ( substr( $attribute_key, 0, 3 ) == 'pa_' ) {
					...
					if ( ! taxonomy_exists( $taxonomy ) ) {
						...
						wc_Product_CSV_Import_Suite::log( sprintf( __('> > Attribute taxonomy "%s" does not exist. Adding it. Nicename: %s', 'rspider-csv-export-import-woo'), $taxonomy, $nicename ) );
						...
						if ( ! $exists_in_db ) {
							...
							wc_Product_CSV_Import_Suite::log( sprintf( __('> > Attribute taxonomy %s already exists in DB.', 'rspider-csv-export-import-woo'), $taxonomy ) );
						}
						...
						foreach ( $raw_terms as $raw_term ) {
							...
							if ( ! $term_id ) {
								...
								if ( ! is_wp_error( $t ) ) {
									...
									wc_Product_CSV_Import_Suite::log( sprintf( __( '> > Inserted Raw Term %s ID = %s', 'rspider-csv-export-import-woo' ), esc_html( $raw_term ), $term_id ) );
								} else {
									wc_Product_CSV_Import_Suite::log( sprintf( __( '> > Failed to import term %s %s', 'rspider-csv-export-import-woo' ), esc_html($raw_term), esc_html($taxonomy) ) );
									break;
								}
							} else {
								wc_Product_CSV_Import_Suite::log( sprintf( __( '> > Raw Term %s ID = %s', 'rspider-csv-export-import-woo' ), esc_html( $raw_term ), $term_id ) );
							}
